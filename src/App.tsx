...
function App() {
    ...
    const [videoFileValue, setVideoFileValue] = React.useState<any>();
    ...
    const handleTrim = async () => {
      if (isScriptLoaded) {
        ...
        ffmpeg.FS(
          'writeFile',
          name,
          await (window as any).FFmpeg.fetchFile(videoFileValue),
        );
    ...
    return (
        <div className='footage-page background'>
            ...
            <div className='add-footage-form container mx-auto padding-6'>
                ...
                <div className='row pt-6'>
                    ...
                    {videoFileValue && (
                        <video className='video-preview' height={338} controls ref={mountedVideoFile}>
                                ...
                                onChange={(values) => {
                                    if (values.length > 0) {
                                        ...
                                        if (mountedVideoFile?.current) {
                                            (mountedVideoFile?.current as any).currentTime = values[0];
                                        }
                                        if (clearTimerRef) {
                                            (clearTimerRef.current as any) = setTimeout(() => {
                                                handleTrim();
                                            }, 3000);
                                        }
                                    }
